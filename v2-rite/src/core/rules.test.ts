import { describe, expect, it } from 'vitest';
import { applyAttack, applyMove } from './battle';
import { getLegalAttacks, getLegalMoves } from './rules';
import { c, has, makeState } from './testHelpers';

describe('движение по рангам (6.5)', () => {
  it('Страж не ходит назад, ходит на 1–3 в пяти направлениях', () => {
    const w = c('player', 'warden', 4, 4);
    const state = makeState([w]);
    const moves = getLegalMoves(state, w.id);
    expect(has(moves, 4, 5)).toBe(false);
    expect(has(moves, 3, 5)).toBe(false);
    expect(has(moves, 4, 3)).toBe(true);
    expect(has(moves, 4, 1)).toBe(true);
    expect(has(moves, 1, 4)).toBe(true);
    expect(has(moves, 2, 2)).toBe(true);
  });

  it('с Сандалиями пилигрима Страж ходит на 4, но атакует на 3', () => {
    const w = c('player', 'warden', 4, 4);
    const state = makeState([w], { relics: ['pilgrimSandals'] });
    const moves = getLegalMoves(state, w.id);
    expect(has(moves, 4, 3)).toBe(true);
    expect(has(moves, 4, 0)).toBe(true);
    expect(has(moves, 4, 5)).toBe(false);

    const far = c('enemy', 'brute', 0, 4);
    const near = c('enemy', 'brute', 1, 4);
    expect(has(getLegalAttacks(makeState([w, far], { relics: ['pilgrimSandals'] }), w.id), 0, 4)).toBe(false);
    expect(has(getLegalAttacks(makeState([w, near], { relics: ['pilgrimSandals'] }), w.id), 1, 4)).toBe(true);
  });

  it('Страж не перепрыгивает через существ', () => {
    const w = c('player', 'warden', 4, 4);
    const ally = c('player', 'warden', 4, 2);
    const state = makeState([w, ally]);
    const moves = getLegalMoves(state, w.id);
    expect(has(moves, 4, 3)).toBe(true);
    expect(has(moves, 4, 1)).toBe(false);
  });

  it('Иерофант ходит во все стороны на 1–5 и не перепрыгивает', () => {
    const h = c('player', 'hierophant', 4, 4);
    const enemy = c('enemy', 'brute', 4, 2);
    const state = makeState([h, enemy]);
    const moves = getLegalMoves(state, h.id);
    expect(has(moves, 4, 5)).toBe(true);
    expect(has(moves, 0, 4)).toBe(true);
    expect(has(moves, 4, 3)).toBe(true);
    expect(has(moves, 4, 1)).toBe(false);
    const attacks = getLegalAttacks(state, h.id);
    expect(has(attacks, 4, 2)).toBe(true);
  });
});

describe('непроходимые клетки (6.5)', () => {
  it('через непроходимую клетку нельзя ни пройти, ни атаковать', () => {
    const h = c('player', 'hierophant', 3, 5);
    const target = c('enemy', 'brute', 3, 1);
    const state = makeState([h, target], { arena: 'well' });
    const moves = getLegalMoves(state, h.id);
    expect(has(moves, 3, 4)).toBe(true);
    expect(has(moves, 3, 3)).toBe(false);
    expect(has(moves, 3, 2)).toBe(false);
    expect(has(getLegalAttacks(state, h.id), 3, 1)).toBe(false);
  });
});

describe('атака и ОД (6.4, 6.6)', () => {
  it('атака тратит все оставшиеся ОД и завершает ход', () => {
    const w = c('player', 'warden', 4, 4);
    const victim = c('enemy', 'brute', 4, 3);
    const other = c('enemy', 'brute', 2, 1);
    const state = makeState([w, victim, other]);
    const { state: next, events } = applyAttack(state, w.id, { x: 4, y: 3 }, () => 0.5);
    expect(next.turn).toBe('enemy');
    expect(events.some((e) => e.t === 'killed' && e.id === victim.id)).toBe(true);
    expect(events.some((e) => e.t === 'turnEnded' && e.side === 'player')).toBe(true);
  });

  it('существо не действует дважды за ход', () => {
    const w = c('player', 'warden', 4, 4);
    const other = c('player', 'warden', 1, 4);
    const enemy = c('enemy', 'brute', 7, 0);
    const state = makeState([w, other, enemy]);
    const { state: next } = applyMove(state, w.id, { x: 4, y: 3 }, () => 0.5);
    expect(next.turn).toBe('player');
    expect(getLegalMoves(next, w.id)).toHaveLength(0);
    expect(getLegalMoves(next, other.id).length).toBeGreaterThan(0);
  });

  it('под Блицкригом зафиксированное существо действует повторно', () => {
    const w = c('player', 'warden', 4, 4);
    const enemy = c('enemy', 'brute', 7, 0);
    const state = makeState([w, enemy], {
      karma: { pendingCard: null, blitzkrieg: { creatureId: null, actionsLeft: 5 }, depressionTurns: 0, discardMessage: null },
    });
    const { state: next } = applyMove(state, w.id, { x: 4, y: 3 }, () => 0.5);
    expect(next.karma.blitzkrieg?.creatureId).toBe(w.id);
    expect(getLegalMoves(next, w.id).length).toBeGreaterThan(0);
  });
});

describe('Панцирь', () => {
  it('Страж не берёт Панциря, Иерофант — берёт', () => {
    const w = c('player', 'warden', 4, 4);
    const h = c('player', 'hierophant', 3, 4);
    const shell = c('enemy', 'shell', 4, 3);
    const state = makeState([w, h, shell]);
    expect(has(getLegalAttacks(state, w.id), 4, 3)).toBe(false);
    expect(has(getLegalAttacks(state, h.id), 4, 3)).toBe(true);
  });
});

describe('посвящение (6.7)', () => {
  it('Страж на последней линии становится Иерофантом и уходит в зону размещения', () => {
    const w = c('player', 'warden', 4, 1, 3);
    const enemy = c('enemy', 'brute', 7, 6);
    const state = makeState([w, enemy]);
    const { state: next, events } = applyMove(state, w.id, { x: 4, y: 0 }, () => 0.5);
    const ordained = events.find((e) => e.t === 'ordained');
    expect(ordained).toBeTruthy();
    const creature = next.creatures.find((cr) => cr.id === w.id)!;
    expect(creature.kind).toBe('hierophant');
    expect(creature.marks).toBe(3);
    expect(creature.cell).toEqual({ x: 0, y: 7 });
  });

  it('второй Страж может ходить сразу после посвящения первого', () => {
    const first = c('player', 'warden', 4, 1);
    const second = c('player', 'warden', 1, 4);
    const enemy = c('enemy', 'brute', 7, 6);
    const state = makeState([first, second, enemy]);
    const { state: next } = applyMove(state, first.id, { x: 4, y: 0 }, () => 0.5);
    expect(next.turn).toBe('player');
    expect(getLegalMoves(next, second.id).length).toBeGreaterThan(0);
  });

  it('отменяется, если свободных клеток размещения нет', () => {
    const w = c('player', 'warden', 4, 1);
    const fillers = [];
    for (let y = 5; y <= 7; y++) for (let x = 0; x < 8; x++) fillers.push(c('player', 'warden', x, y));
    const enemy = c('enemy', 'brute', 7, 3);
    const state = makeState([w, ...fillers, enemy]);
    const { state: next, events } = applyMove(state, w.id, { x: 4, y: 0 }, () => 0.5);
    expect(events.some((e) => e.t === 'ordainCancelled' && e.id === w.id)).toBe(true);
    const creature = next.creatures.find((cr) => cr.id === w.id)!;
    expect(creature.kind).toBe('warden');
    expect(creature.cell).toEqual({ x: 4, y: 0 });
  });
});
